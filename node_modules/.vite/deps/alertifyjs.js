...
//#region node_modules/alertifyjs/build/alertify.js
var require_alertify = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	/**
	...
	(function(window) {
		...
		*/
		var transition = function() {
			...
			for (t in transitions) if (document.documentElement.style[t] !== void 0) {
				type = transitions[t];
				supported = true;
				break;
			}
		...
		*/
		var dialog = function() {
			var usedKeys = [], tabindex = false, isSafari = window.navigator.userAgent.indexOf("Safari") > -1 && window.navigator.userAgent.indexOf("Chrome") < 0, templates = {
			...
			*/
			function initialize(instance) {
				if (!instance.__internal) {
					...
					for (var opKey in internal.options) if (setup.options[opKey] !== void 0) instance.set(opKey, setup.options[opKey]);
					else if (alertify.defaults.hasOwnProperty(opKey)) instance.set(opKey, alertify.defaults[opKey]);
					else if (opKey === "title") instance.set(opKey, alertify.defaults.glossary[opKey]);
			...
			*/
			function updateTransition(instance, value, oldValue) {
				if (isString(oldValue)) removeClass(instance.elements.root, classes.prefix + oldValue);
			...
			*/
			function updateTransitionOff(instance) {
				if (instance.get("transitionOff")) addClass(instance.elements.root, classes.noTransition);
				else removeClass(instance.elements.root, classes.noTransition);
			}
			/**
			...
			*/
			function updateBasicMode(instance) {
				if (instance.get("basic")) addClass(instance.elements.root, classes.basic);
				else removeClass(instance.elements.root, classes.basic);
			}
			/**
			...
			*/
			function updateFramelessMode(instance) {
				if (instance.get("frameless")) addClass(instance.elements.root, classes.frameless);
				else removeClass(instance.elements.root, classes.frameless);
			}
			/**
			...
			*/
			function bringToFront(event, instance) {
				for (var x = openDialogs.indexOf(instance) + 1; x < openDialogs.length; x += 1) if (openDialogs[x].isModal()) return;
			...
			function optionUpdated(instance, option, oldValue, newValue) {
				switch (option) {
					...
					case "transition":
						...
						break;
					case "transitionOff": updateTransitionOff(instance);
				}
			...
			*/
			function commandsClickHandler(event, instance) {
				switch (event.srcElement || event.target) {
					...
					case instance.elements.commands.maximize:
						...
						break;
					case instance.elements.commands.close: triggerClose(instance);
				}
				return false;
			}
			/**
			...
			*/
			function updatePinnable(instance) {
				if (instance.get("pinnable")) addClass(instance.elements.root, classes.pinnable);
				else removeClass(instance.elements.root, classes.pinnable);
			}
			/**
			...
			*/
			function updateMaximizable(instance) {
				if (instance.get("maximizable")) addClass(instance.elements.root, classes.maximizable);
			...
			function triggerCallback(instance, check) {
				if (Date.now() - callbackTS > 200 && (callbackTS = Date.now())) for (var idx = 0; idx < instance.__internal.buttons.length; idx += 1) {
					...
					if (!button.element.disabled && check(button)) {
						...
						if (typeof instance.callback === "function") instance.callback.apply(instance, [closeEvent]);
				...
				if (resetTarget) resetTarget.focus();
					...
					switch (typeof focus.element) {
						...
						case "string":
							...
							break;
						case "function": element = focus.element.call(instance);
					}
			...
			function onReset(event, instance) {
				...
				if (instance) {
					if (instance.isModal()) {
						...
						if (target === firstReset && !within || target === lastReset && lastFocusedElement === firstReset) return;
			...
			*/
			function resizeElement(event, element, pageRelative) {
				...
				if (!isNaN(startingLeft)) {
					...
					if (isRTL) diff *= -1;
					if (element.offsetWidth > startingWidth) element.style.left = startingLeft + diff + "px";
					else if (element.offsetWidth >= minWidth) element.style.left = startingLeft - diff + "px";
				}
			}
			/**
				...
				*/
				show: function(modal, className) {
					...
					if (!this.__internal.isOpen) {
						...
						bindEvents(this);
						if (modal !== void 0) this.set("modal", modal);
						saveScrollPosition();
						ensureNoOverflow();
		...
		var notifier = (function() {
			var element, openInstances = [], classes = defaults.notifier.classes, baseClass = classes.base;
			/**
			...
			*/
			function initialize(instance) {
				if (!instance.__internal) {
					...
					if ("transitionOff" in defaults.notifier ? defaults.notifier.transitionOff : defaults.transitionOff) baseClass = classes.base + " ajs-no-transition";
			...
			function updatePosition(instance) {
				...
				switch (instance.__internal.position) {
					...
					case "bottom-center":
						...
						break;
					default:
					case "bottom-right": addClass(element, classes.bottom + " " + classes.right);
				}
			}
			/**
			...
			*/
			...
			return {
				...
				*/
				setting: function(key, value) {
					initialize(this);
					if (typeof value === "undefined") return this.__internal[key];
					else switch (key) {
						case "position":
							this.__internal.position = value;
							updatePosition(this);
							break;
						case "delay": this.__internal.delay = value;
					}
					return this;
				},
				/**
				...
				*/
				dismissAll: function(except) {
					...
					for (var x = 0; x < clone.length; x += 1) {
						...
						if (except === void 0 || except !== instance) instance.dismiss();
					}
				}
			};
		})();
		...
		function Alertify() {
			...
			*/
			function register(name, Factory, transient, base) {
				...
				if (base !== void 0) definition.factory = function() {
			...
			return {
				...
				*/
				dialog: function(name, Factory, transient, base) {
					...
					if (transient) this[name] = function() {
				...
				*/
				closeAll: function(except) {
					...
					for (var x = 0; x < clone.length; x += 1) {
						var instance = clone[x];
						if (except === void 0 || except !== instance) instance.close();
					}
				},
				/**
				...
				settingUpdated: function(key, oldValue, newValue) {
					switch (key) {
						case "message":
							...
							break;
						case "label": if (this.__internal.buttons[0].element) this.__internal.buttons[0].element.innerHTML = newValue;
					}
				},
				...
				task: function(event, self) {
					if (self.isOpen()) {
						...
						if (autoConfirm.duration === -1) {
							...
							if (typeof self.callback === "function") self.callback.apply(self, [closeEvent]);
				...
				settingUpdated: function(key, oldValue, newValue) {
					switch (key) {
						...
						case "reverseButtons":
							...
							break;
						case "defaultFocus": this.__internal.focus.element = newValue === "ok" ? 0 : 1;
					}
				},
				callback: function(closeEvent) {
					...
					switch (closeEvent.index) {
						case 0:
							...
							break;
						case 1: if (typeof this.get("oncancel") === "function") {
		...
		*/
				...
				settingUpdated: function(key, oldValue, newValue) {
					switch (key) {
						...
						case "labels":
							...
							break;
						case "reverseButtons": if (newValue === true) this.elements.buttons.primary.appendChild(this.__internal.buttons[0].element);
						else this.elements.buttons.primary.appendChild(this.__internal.buttons[1].element);
					}
				},
				callback: function(closeEvent) {
		...
		if (typeof module === "object" && typeof module.exports === "object") module.exports = alertify;
